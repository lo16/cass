...
for i in range(num_users):
	...
	# find the first process ID for this machines of our IP address
...
#if ip is not in config file, kill yourself
if not first_id_for_this_ip_found:
...
while 1:
	...
	if(len(command) > 0 and command[0] in commands.keys() and len(command) == commands[command[0]]):
		# process the command
